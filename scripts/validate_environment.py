...
# Canonical names only. settings.py still accepts the legacy STORAGE_* aliases,
# but new deployments must be configured with the canonical S3_* names.
REQUIRED_PRODUCTION = [
    "PUBLIC_API_URL", "PUBLIC_APP_URL", "CORS_ORIGINS", "STORAGE_BACKEND",
    "S3_BUCKET", "S3_ACCESS_KEY", "S3_SECRET_KEY", "EMAIL_FROM",
]
SECRET_NAMES = {
    "SECRET_KEY", "DATA_ENCRYPTION_KEY", "AI_API_KEY", "TAVILY_API_KEY", "BRAVE_API_KEY",
    "SERPER_API_KEY", "SEMANTIC_SCHOLAR_API_KEY", "S3_ACCESS_KEY", "S3_SECRET_KEY",
    "GOOGLE_CLIENT_SECRET", "APPLE_PRIVATE_KEY",
...
def validate(env: dict[str, str], environment: str) -> tuple[list[str], list[str]]:
    ...
    if production:
        ...
        if get("STORAGE_BACKEND") != "s3":
            errors.append("STORAGE_BACKEND must be s3: generated PDFs need durable storage, "
                          "not ephemeral application-server disk")
        # Google sign-in, when enabled, needs at least one verifiable audience.
        if truthy(get("GOOGLE_AUTH_ENABLED")):
            if not any(get(n) for n in ("GOOGLE_ANDROID_CLIENT_ID",
                                        "GOOGLE_IOS_CLIENT_ID",
                                        "GOOGLE_WEB_CLIENT_ID")):
                errors.append("GOOGLE_AUTH_ENABLED is set but no GOOGLE_*_CLIENT_ID is "
                              "configured; ID-token verification would reject every login")
        if truthy(get("APPLE_AUTH_ENABLED")) and not get("APPLE_CLIENT_ID"):
            errors.append("APPLE_AUTH_ENABLED is set but APPLE_CLIENT_ID is missing")
